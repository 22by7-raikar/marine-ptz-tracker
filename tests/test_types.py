...
def test_detection_optional_track_id_is_backward_compatible_and_strict() -> None:
    assert Detection("boat", 0.9, 0.0, 0.0, 1.0, 1.0).track_id is None
    assert Detection("boat", 0.9, 0.0, 0.0, 1.0, 1.0, 7).track_id == 7

    for invalid in (True, -1, 1.5):
        with pytest.raises(ValueError, match="track_id"):
            Detection("boat", 0.9, 0.0, 0.0, 1.0, 1.0, invalid)  # type: ignore[arg-type]
